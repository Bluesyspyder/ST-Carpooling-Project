...
const Profile = () => {
  ...
  setEditForm({
    ...
    role:             user.role || 'passenger',   // <-- ADD THIS
    bio:              user.bio || '',
  ...
  /* Bottom nav destinations — same shape as Dashboard / SearchRide so the app shell stays consistent. */
  const navItems = [
    {
      href: '/',
      label: 'Home',
  ...
  try {
    ...
    const response = await fetch("/api/profile/change-role", {
      method: "PUT",
      headers: {
      ...
      <div className="w-full pb-32">
        ...
        <div className="sticky top-0 z-30 glass-panel rounded-none border-x-0 border-t-0 px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-3 flex items-center justify-between backdrop-blur-xl">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-full bg-emerald-500/10 border-2 border-emerald-500/30 flex items-center justify-center text-sm font-bold text-emerald-400 overflow-hidden flex-shrink-0">
              {user.profileImage
                ? <img src={user.profileImage} alt="Profile" className="w-full h-full object-cover" />
                : <>{user.firstName?.[0]}{user.lastName?.[0]}</>}
            </div>
            <div className="min-w-0">
              <p className="text-[var(--primary-base)] font-bold uppercase tracking-widest text-[10px] mb-0.5">Profile</p>
              <h1 className="text-sm sm:text-base font-bold text-[var(--text-primary)] truncate">{user.firstName} {user.lastName}</h1>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="flex-shrink-0 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest border border-red-500/40 text-red-400 hover:bg-red-500/10 transition-colors"
          >
            Sign Out
          </button>
        </div>

        <div className="px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 pt-6">
        ...
        <div className="glass-panel p-6 sm:p-10 space-y-8 shadow-sm shadow-[var(--border-glow)]">
          ...
          <div className="mt-6 p-5 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)]">
  <h3 className="text-sm font-bold uppercase tracking-widest text-[var(--text-primary)] mb-2">
    Account Type
  </h3>

  <p className="text-sm text-[var(--text-secondary)] mb-4">
    Current Role:
    <span className="font-semibold ml-2 text-[var(--primary-base)]">
      {user.role === "hybrid" ? "Rider" : "Co-Rider"}
    </span>
  </p>
