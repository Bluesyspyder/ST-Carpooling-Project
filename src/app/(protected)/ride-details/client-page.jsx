...
const RideDetails = () => {
  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.message || 'Failed to request booking. Please try again.';
        ...
        <div className="sticky top-0 z-30 glass-panel rounded-none border-x-0 border-t-0 px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-3 flex items-center justify-between backdrop-blur-xl">
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className="w-9 h-9 rounded-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] flex items-center justify-center text-[var(--text-primary)] flex-shrink-0"
              aria-label="Go back"
            >
            ...
            <div className="min-w-0">
              <p className="text-[var(--primary-base)] font-bold uppercase tracking-widest text-[10px] mb-0.5">Ride Details</p>
          ...
          <span className={`flex-shrink-0 px-2.5 py-1 text-[10px] font-bold border rounded-full capitalize ${statusBadge(ride.rideStatus?.toLowerCase())}`}>
            {ride.rideStatus}
          </span>
        </div>

        <div className="px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 pt-6 space-y-6">

          {/* SOS Alert Banner */}
          {sosAlert && (
            <div className="bg-red-900 border-2 border-red-500 rounded-xl p-4 shadow-[0_0_20px_rgba(239,68,68,0.4)] animate-pulse">
