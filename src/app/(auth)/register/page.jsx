...
const Register = () => {
  ...
  try {
    ...
    const response = await fetch(
      `/api/vehicle-lookup?vehicle_no=${encodeURIComponent(
...
if (!response.ok) {
  throw new Error(result.message || "Vehicle lookup failed");
}
...
setFormData(prev => {
  const updated = {
    ...prev,
    vehicleOwner: vehicle.rc_owner_name || "",
    vehicleBrand: vehicle.rc_maker_desc || "",
    vehicleModel: vehicle.rc_maker_model || "",
    vehicleType: vehicle.rc_fuel_desc || "",
    vehicleColour: vehicle.rc_color || vehicle.rc_colour || "",
    seatCount: Number(vehicle.rc_seat_cap) || prev.seatCount,
  };
  ...
  return updated;
    ...
    console.error(err);
    setError(err.message || "Unable to fetch vehicle details.");
  } finally {
