      ...
      <div className="flex gap-2 min-w-max">
        ...
        {showCurrentLocation && (
          <button
            ...
            className="flex items-center gap-2 bg-violet-600/20 border border-violet-500/30 hover:border-violet-500/60 text-violet-300 rounded-lg px-3 py-2 text-sm transition-all whitespace-nowrap"
          >
            <span>📍</span>
        ...
        {savedAddresses.map((addr) => (
          <button
            ...
            className="flex items-center gap-2 bg-slate-700/60 border border-slate-600/40 hover:border-slate-500 text-slate-200 rounded-lg px-3 py-2 text-sm transition-all whitespace-nowrap"
          >
            <span>{addr.icon || '⭐'}</span>
        ...
        {top5Recent.map((addr, i) => {
          ...
          return (
            <button
              key={i}
              onClick={() => onSelect({ address: addr.address, latitude: addr.latitude, longitude: addr.longitude })}
              className="flex flex-col items-start bg-slate-700/40 border border-slate-600/30 hover:border-slate-500 text-slate-300 rounded-lg px-3 py-2 text-xs transition-all whitespace-nowrap"
            >
              <span className="font-medium text-sm text-slate-200 truncate max-w-28">{label}</span>
        ...
        {top5Frequent.map((addr, i) => {
          ...
          return (
            <button
              key={`freq-${i}`}
              onClick={() => onSelect({ address: addr.address, latitude: addr.latitude, longitude: addr.longitude })}
              className="flex flex-col items-start bg-slate-700/40 border border-yellow-500/20 hover:border-yellow-500/50 text-slate-300 rounded-lg px-3 py-2 text-xs transition-all whitespace-nowrap"
            >
              <span className="font-medium text-sm text-slate-200 truncate max-w-28">{label}</span>
