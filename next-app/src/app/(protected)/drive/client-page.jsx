...
const DriveMode = () => {
  ...
  if (loading) {
    return (
      <div className="h-screen bg-[var(--bg-default)] flex flex-col items-center justify-center gap-5 text-[var(--text-secondary)]">
        <div className="w-12 h-12 border-3 border-emerald-400 border-t-transparent rounded-full animate-spin border-[3px]" />
        <div className="text-center">
          <p className="text-sm font-semibold text-[var(--text-primary)]">Calculating optimal route…</p>
          <p className="text-xs text-[var(--text-muted)] mt-1">Running TSP optimization on all passenger pickups</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="h-screen bg-[var(--bg-default)] flex flex-col items-center justify-center gap-5 px-4 text-center">
        ...
        <div>
          <p className="text-[var(--text-primary)] font-bold text-lg">{error}</p>
          <p className="text-[var(--text-muted)] text-sm mt-1">You may not be the driver of this ride.</p>
        </div>
        <button
  ...
  return (
    <div className="h-screen w-screen bg-[var(--bg-default)] flex flex-col overflow-hidden">

      {/* ── Top Bar ── */}
      <div className="flex items-center justify-between px-4 py-3 bg-[var(--bg-surface)]/90 backdrop-blur border-b border-[var(--border-subtle)] z-20 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-4 text-center">
            <div>
              ...
              <p className="text-[10px] text-[var(--text-muted)]">total distance</p>
            </div>
            <div>
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <div className="w-full lg:w-96 bg-[var(--bg-surface)] border-t lg:border-t-0 lg:border-l border-[var(--border-subtle)] flex flex-col overflow-hidden flex-shrink-0" style={{ maxHeight: '50vh', minHeight: '240px' }}>
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl border transition-all duration-300 ${
                    isPickedUp
                      ? 'bg-[var(--bg-surface-hover)]/30 border-[var(--border-subtle)]/40 opacity-50'
                      : isCurrent
                      ? 'bg-emerald-500/10 border-emerald-500/30 shadow-sm shadow-emerald-500/20'
                      : 'bg-[var(--bg-surface-hover)]/50 border-[var(--border-subtle)]'
                  }`}
                >
          ...
          {/* Bottom actions */}
          <div className="p-4 border-t border-[var(--border-subtle)] flex-shrink-0 space-y-2">
