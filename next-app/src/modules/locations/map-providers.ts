...
export const autocompleteWithOla = async (query) => {
  ...
  try {
    ...
    let data;
    try { data = JSON.parse(text); } catch (e) { console.error('[Ola Maps] Autocomplete JSON Parse Error:', e); data = null; }
    ...
    if (!res.ok || (data && data.status && data.status !== 'ok' && data.status !== 'zero_results')) {
      ...
      // If Ola Maps returns 401 (Invalid API Key), do NOT throw a 401 to our frontend, otherwise it triggers a logout loop
      const statusCode = res.status === 401 ? 502 : (res.status === 200 ? 500 : res.status);
      throw new ApiError(statusCode, `Ola Maps Autocomplete Error: ${msg}`);
    }
...
export const geocodeWithOla = async (address) => {
  ...
  try {
    ...
    const text = await res.text();
    let data;
    try { data = JSON.parse(text); } catch (e) { console.error('[Ola Maps] Geocode JSON Parse Error:', e); data = null; }
...
export const reverseGeocodeWithOla = async (lat, lng) => {
  ...
  try {
    ...
    const text = await res.text();
    let data;
    try { data = JSON.parse(text); } catch (e) { console.error('[Ola Maps] Reverse Geocode JSON Parse Error:', e); data = null; }
