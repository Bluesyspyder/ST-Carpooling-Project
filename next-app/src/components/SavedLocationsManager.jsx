  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-slate-200 font-semibold flex items-center gap-2"><span>📌</span> Saved Locations</h3>
        <button onClick={openAdd} className="bg-violet-600 hover:bg-violet-500 text-white text-sm px-4 py-2 rounded-lg transition-colors">
          + Add Location
        </button>
      </div>
      ...
      {addresses.length === 0 ? (
        <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-8 text-center">
          ...
          <p className="text-slate-300 font-medium">No saved locations yet</p>
          <p className="text-slate-500 text-sm mt-1">Add your Home and Office for quick access.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {addresses.map((addr) => (
            <div key={addr._id} className="flex items-center justify-between bg-slate-800/40 border border-slate-700/50 rounded-xl px-4 py-3">
              <div className="flex items-center gap-3 min-w-0">
                <span className="text-2xl flex-shrink-0">{addr.icon || '⭐'}</span>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-slate-200 font-medium text-sm">{addr.label}</span>
                  ...
                  <p className="text-slate-500 text-xs truncate">{addr.address}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                <button onClick={() => setViewLocation(addr)} className="text-slate-400 hover:text-slate-200 text-xs px-2 py-1 rounded transition-colors">Map</button>
                <button onClick={() => handleSetDefault(addr._id)} className="text-slate-400 hover:text-violet-300 text-xs px-2 py-1 rounded transition-colors">Default</button>
                <button onClick={() => openEdit(addr)} className="text-slate-400 hover:text-blue-300 text-xs px-2 py-1 rounded transition-colors">Edit</button>
                <button onClick={() => handleDelete(addr._id)} className="text-slate-400 hover:text-red-300 text-xs px-2 py-1 rounded transition-colors">Delete</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={(e) => e.target === e.currentTarget && setShowModal(false)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <h3 className="text-lg font-semibold text-white mb-5">{editId ? 'Edit Location' : 'Add Location'}</h3>
            ...
            <div className="mb-4">
              ...
              <input value={form.label} onChange={(e) => setForm((f) => ({ ...f, label: e.target.value }))}
                ...
                className="w-full bg-slate-800/60 border border-slate-600/50 rounded-lg px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-violet-500 text-sm" />
            </div>
            ...
            {form.latitude && (
              <div className="mb-4">
                <label className="block text-sm text-slate-400 mb-2">Confirm pin position</label>
                <MapPreview
                  location={form}
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={handleSave} disabled={saving}
                className="bg-violet-600 hover:bg-violet-500 text-white text-sm px-5 py-2 rounded-lg transition-colors disabled:opacity-50">
                {saving ? 'Saving…' : 'Save'}
              </button>
            </div>
          </div>
