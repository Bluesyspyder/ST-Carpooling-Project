...
const VEHICLE_CONFIG = {
  ev:      { label: 'Electric Vehicle', icon: 'ΓÜí', color: '#10b981', multiplier: '├ù1.8 EV Bonus' },
  petrol:  { label: 'Petrol',           icon: 'Γ¢╜', color: '#f59e0b', multiplier: null },
  diesel:  { label: 'Diesel',           icon: '≡ƒ¢ó',  color: '#6366f1', multiplier: null },
};

/**
 * Animated number counter ΓÇö counts from 0 to `target` in `durationMs`.
 ...
 *   isOpen               ΓÇö boolean
 *   onClose              ΓÇö fn()
 *   driverCreditsEarned  ΓÇö number (from completed ride API response)
 *   totalEmissionSavedKg ΓÇö number (from completed ride API response)
 *   routeDistance        ΓÇö number km
 *   vehicleType          ΓÇö 'ev' | 'petrol' | 'diesel'
 *   passengerCount       ΓÇö number (default 1)
 */
const DemoCompletionModal = ({
  ...
  // Animated counts ΓÇö only run when modal is open
    ...
    >
      ...
      >
        ...
        >
          ...
          <h2 className="text-2xl font-black text-white mb-1">Ride Completed! ≡ƒÄë</h2>
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl" style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.15)' }}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center text-lg">≡ƒÜù</div>
              <div>
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-slate-500">≡ƒî┐ credits</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl" style={{ background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.15)' }}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-indigo-500/20 flex items-center justify-center text-lg">≡ƒºæ</div>
              <div>
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">
                  Passenger{passengerCount > 1 ? `s (├ù${passengerCount})` : ''}
                </p>
                <p className="text-slate-200 text-sm font-bold">Eco-Credits Earned</p>
              </div>
            </div>
            <div className="text-right">
              ...
              <p className="text-[10px] text-slate-500">≡ƒî┐ each</p>
            </div>
          </div>
          ...
          >
            <div className="flex-1">
              <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-0.5">COΓéé Emissions Saved</p>
              <p className="text-amber-400 text-xl font-black">{co2Count} kg</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-slate-500 mb-0.5">Equivalent to</p>
              <p className="text-amber-300 font-bold text-sm">≡ƒî│ {treesEquivalent} trees/month</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 pb-6">
          ...
          <p className="text-center text-[10px] text-slate-600 mt-3">
            All numbers are live ΓÇö pulled from the real API response.
          </p>
        </div>
      </div>
    </div>
